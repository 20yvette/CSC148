...
class TestDescendants(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.apple = Person('Apple')
        self.david = Person('David')

        self.alice.children = [self.bob, self.charlie]
        self.bob.children = [self.apple, self.david]

    def test_descendants_simple(self):
        self.assertEqual(self.alice.count_descendants(), 4)

    def test_descendants_simple_two(self):
        self.assertEqual(self.bob.count_descendants(), 2)
    
    def test_descendants_none(self):
           self.assertEqual(self.apple.count_descendants(), 0)
