    ...
    def is_heap(self, index=1):
        ...
        if self.is_empty():
            ...
            if index_item >= left_item and index_item >= right_item:
                return self.is_heap(left) and self.is_heap(right)
            else:
                return False
        elif left <len(self.items):
            index_item = self.items[index]
...
def build_heap_2(lst):
    ...
    """
    heap_2 = Heap()
    heap_2.items = [None] + lst[:]
    index = len(heap_2.items) - 1
    while index > 0:
        heap_2.bubble_down(index)
        index -= 1
