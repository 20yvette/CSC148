...
def binary_search(lst, item):
    ...
    if len(lst) == 0:
        ...
        #if item is smaller or equal to than the item to the left of the middle of the list, then side == true, and do binary search on the left side
        #else if its bigger, search the right side.
        if item <= lst[len(lst)//2 - 1]:
            side = True
        else:
            side = False
        return binary_search(return_half(lst, side), item)
...
def print_left(lst, left=0, right=None):
    ...
    #(left+right)//2 = left portion of lst[left:right]
    for i in lst[left:(left+right)//2]:
        print(i)
...
def indexed_binary_search(lst, item, left=0, right=None):
    ...
    if lst == []:
        return False
    elif (left+right)//2 >= len(lst):
        return False
    mid = lst[((left+right)//2) + 1]
    elif mid == item:
        return True    
    elif (right - left == 1):
        return False
        ...
        left = (left+right//2) 
        right = right
        return indexed_binary_search(lst,item,left, right)
    elif mid > item:
        left = left
        right = (left+right)//2
        return indexed_binary_search(lst,item,left,right)
